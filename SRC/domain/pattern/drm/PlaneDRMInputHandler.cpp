                      ...
                      my_mesher)
{
   ...
   *  included in the files where the first one is the zero time step
  ...
  this->f1buffer = new double[3 * (this->cacheValue + 4) * this->fileData[0]];  //?huangjf 20190903: cache 4 more steps than the designated valus
...
void PlaneDRMInputHandler::populateBuffers()
{
  ...
  if (this->initial)
  {
    ...
    for (int i = 3 * temp; i < 3 * (cacheValue + 4) * temp; i++) //[20190910 huangjf] considering i starts from 3*temp,  cacheValue+4 actually caches 3 more time steps, not 4 more time steps
    {
      ifile1 >> dataIn;
      f1buffer[i] = dataIn;
    }
  ...
  else
  {
    ...
    for (int k = 3; k < (cacheValue + 4); k++)  //?huangjf 20190911: for (int k = 3; k < (rem + 4); k++)
    {
      for (int i = 0; i < 3 * nd1; i++)
      {
        ifile5a >> dataIn;
    ...
    globalCounter += cacheValue + 1; //?huangjf 20190911: globalCounter += rem + 1;

    //update time buff
    timeBuf[0] = timeBuf[this->cacheValue + 1];
    timeBuf[1] = timeBuf[this->cacheValue + 2];
    timeBuf[2] = timeBuf[this->cacheValue + 3];
    for (int i = 0; i < this->cacheValue + 1; i++) //?huangjf 20190911: for (int i = 0; i < this->rem + 1; i++)
      timeBuf[3 + i] = timeBuf[2 + i] + this->deltaT;
  }
}
...
void PlaneDRMInputHandler::computeHistory(Element *eletag, double time, Vector &U, Vector &Ud, Vector &Udd, bool updateDm1)
{
  ...
  //[20190905 huangjf] the same as the linear interpolation of velocities, except here the acceleration at time m1, p1 is calculated first, then the acceleration at internal time can be interpolated
